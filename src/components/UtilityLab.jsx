...
const PowerBIView = ({ metric }) => {
  // Classic aggregated bar chart logic
  const dataMap = {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex-1 flex items-end justify-between gap-3 h-48 px-4 w-full">
      {data.map((val, i) => (
...
const GA4View = ({ metric }) => {
  // Real-time event stream / Funnel logic
  const eventMap = {
    velocity: [
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex-1 flex flex-col justify-center gap-4 h-48 w-full px-2">
      {events.map((ev, i) => (
        <motion.div 
          key={`ga4-${metric}-${i}`}
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <span className="font-mono text-[10px] text-stone-300">{ev.event}</span>
          </div>
          <div className="flex items-center gap-6 text-right">
            <span className="font-mono text-[10px] font-bold text-white">{ev.users}</span>
            <span className="font-mono text-[9px] text-stone-500 w-10">{ev.rate}</span>
          </div>
        </motion.div>
      ))}
    </motion.div>
...
const GTMView = ({ metric }) => {
  // Data Layer & Tag Inspector Console
  const tagMap = {
    velocity: [
      { name: "Salesforce_Lead_Sync", status: "Succeeded", type: "Custom HTML" },
      { name: "Hubspot_Form_Submit", status: "Succeeded", type: "Image Tag" }
    ],
    attribution: [
      { name: "LinkedIn_Insight_Tag", status: "Succeeded", type: "Template" },
      { name: "Google_Ads_Conversion", status: "Succeeded", type: "Template" }
    ],
    health: [
      { name: "Cookie_Consent_Update", status: "Fired", type: "Trigger" },
      { name: "DataLayer_Validation", status: "Passed", type: "Variable" }
    ]
  };
  const tags = tagMap[metric];

  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex-1 flex gap-4 h-48 w-full">
      <div className="w-1/2 border border-stone-800 bg-stone-950 rounded-lg p-4 flex flex-col">
        ...
        <div className="space-y-2 flex-1">
          {tags.map((tag, i) => (
            <motion.div key={`gtm-tag-${i}`} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.2 }} className="flex items-center gap-2">
              ...
              <span className="font-mono text-[9px] text-stone-300">{tag.name}</span>
            </motion.div>
          ))}
        </div>
      </div>
      <div className="w-1/2 border border-stone-800 bg-stone-950 rounded-lg p-4 flex flex-col font-mono text-[8px] text-emerald-400 overflow-hidden relative">
...
const ExecutivePerformanceHub = () => {
  ...
  const metricConfig = {
    velocity: { label: "Pipeline Velocity" },
    attribution: { label: "Attribution Accuracy" },
    health: { label: "Data Ecosystem Health" }
  };
  ...
  return (
    ...
    >
      <div className="relative z-10 grid grid-cols-1 lg:grid-cols-12 gap-12">
        ...
        <div className="lg:col-span-4 flex flex-col justify-between">
          <div>
            <div className="flex items-center gap-3 mb-6">
              ...
              <h4 className="font-bold text-xl uppercase tracking-tighter">Global Performance Hub</h4>
            </div>
            ...
            <div className="space-y-2">
              {Object.keys(metricConfig).map((metric) => (
                <button 
                  ...
                  className={`w-full text-left px-5 py-3 rounded-xl font-mono text-[10px] font-bold uppercase tracking-widest transition-all duration-300 flex justify-between items-center ${activeMetric === metric ? 'bg-white text-stone-900 shadow-xl' : 'bg-white/5 text-stone-500 hover:bg-white/10'}`}
                >
                  <span>{metricConfig[metric].label}</span>
                  {activeMetric === metric && <Activity size={12} className="animate-pulse" />}
                </button>
              ))}
            </div>
          </div>
        ...
        <div className="lg:col-span-8 bg-black/40 rounded-3xl p-8 border border-white/5 flex flex-col h-[360px] relative">
          <div className="flex justify-between items-center mb-8">
            ...
            <div className="text-right">
               ...
               <span className={`font-mono text-[10px] font-bold uppercase ${currentTheme.text}`}>{metricConfig[activeMetric].label}</span>
            </div>
          </div>
...
const tools = [
  ...
  { 
    ...
    demo: <div className="mt-4 p-4 bg-stone-950 rounded-xl flex items-center justify-center border border-stone-800">
  ...
  { 
    ...
    demo: <div className="mt-4 p-4 border border-dashed border-stone-200 rounded-xl text-center font-mono text-[8px] text-stone-300 uppercase tracking-[0.2em]">
    ...
    <section id="lab" className="py-32 px-6 bg-paper relative overflow-hidden">
      <div className="max-w-6xl mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-20 gap-8">
          ...
          <div className="px-4 py-2 bg-white border border-stone-200 rounded-full shadow-sm">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {tools.map((tool, i) => (
            <motion.div 
              ...
              viewport={{ once: true }}
              className="p-8 bg-white border border-stone-200 rounded-[32px] flex flex-col justify-between group hover:border-zapier transition-all duration-500 hover:shadow-2xl hover:shadow-orange-500/5"
            >
              <div>
